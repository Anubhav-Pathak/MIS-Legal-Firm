export async function postRead(pageNumber: Number, company: string, limit: Number){
...
export async function getFilter(columns : {}) {
    const params = new URLSearchParams(columns).toString();
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/read/filter?${params}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json'
        }
